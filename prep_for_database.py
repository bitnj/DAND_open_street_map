        ...
        for key, value in tag.attrib.items():
            ...
            if is_ascii(key) and is_ascii(value):
                ...
                if key == 'k':
                    ...
                    # split the value on any ; in the string
                    val_strs = value.split(';')[0]
                    # currently only looks for , in the string to address issue
                    # on import into MySQL.  Could be expanded to check for
                    # additional problem characters
                    m = PROBLEMCHARS.search(val_strs)
                    if m:
                        val_strs = val_strs.replace(',', '')
